        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 py-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="text-center mb-10">
              ...
              <p className="text-muted-foreground mt-2">Transform any article into a broadcast-quality video</p>
            </div>

            {/* Video Player Placeholder */}
            <div className="glass rounded-2xl overflow-hidden mb-8 aspect-video flex items-center justify-center relative">
              {generated ? (
                <div className="absolute inset-0 flex items-center justify-center bg-background/50">
                  <div className="text-center">
                    <Video className="w-16 h-16 text-gold mx-auto mb-3" />
                    <p className="text-sm text-muted-foreground">AI-generated news video ready</p>
                ...
                <div className="text-center px-10 w-full">
                  ...
                  <p className="text-sm text-muted-foreground mb-3">Generating AI video...</p>
                  <div className="w-full max-w-md mx-auto bg-secondary/50 rounded-full h-2 overflow-hidden">
                    <motion.div
                      className="h-full rounded-full"
                      style={{ background: "var(--gradient-gold)", width: `${progress}%` }}
                    />
                  </div>
                  <p className="text-xs text-muted-foreground mt-2">{progress}% — {progress < 30 ? "Analyzing article..." : progress < 60 ? "Generating visuals..." : progress < 90 ? "Adding narration..." : "Finalizing..."}</p>
                </div>
              ) : (
                <div className="text-center">
                  <Video className="w-16 h-16 text-muted-foreground/30 mx-auto mb-3" />
                  <p className="text-sm text-muted-foreground">Select an article and generate a video</p>
                </div>
              )}
            </div>
            ...
            <div className="text-center mb-10">
              <button
                ...
                disabled={generating}
                className="px-8 py-3.5 rounded-xl font-semibold text-primary-foreground text-base inline-flex items-center gap-2 disabled:opacity-50"
